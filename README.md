...
- 重新整理设置窗口的信息架构，改用「选歌 / 外观 / 关于」顶部 Tab，并统一选歌相关文案。
- 将开机自启动与查看日志移到设置页右上角，简化低频设置入口。
- 优化歌曲来源区域布局，并禁止平台下拉框因鼠标滚轮误切换。

## v1.1.1 更新

- 网易云音乐、QQ 音乐和酷狗音乐支持直接粘贴歌单/专辑分享链接，加载后会自动保存规范化 ID 或短码。
...
- 按下默认快捷键 `Alt+D`（可在设置中修改），或左键单击系统托盘图标，从随机候选歌曲中选一首播放。
- 选择一首歌
- 然后听就完事了！
...
[本仓库 Releases](https://github.com/Narcissu-s1/DiscoAS/releases) 中有 Windows 打包文件时，可以下载压缩包，解压后双击 `DiscoAS.exe`。`v1.1.2` 标签包含本页所述源码；没有打包文件时，请按下方“Python 环境下运行”从源码启动。
